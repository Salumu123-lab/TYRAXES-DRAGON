...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  if (!text) return m.reply(`https://whatsapp.com/channel/0029VaNPPwR30LKQk437x51Q *[❗𝐈𝐍𝐅𝐎❗] command usage*\n\n*${usedPrefix + command}* hola @${m.sender.split`@`[0]} a`, null, { mentions: [m.sender] })
  let cm = copy(m)
  let who
  if (text.includes('@0')) who = '0@s.whatsapp.net'
  else if (m.isGroup) who = cm.participant = m.mentionedJid[0]
  else who = m.chat
  if (!who) return m.reply(`https://whatsapp.com/channel/0029VaNPPwR30LKQk437x51Q *[❗𝐈𝐍𝐅𝐎❗] command usage*\n\n*${usedPrefix + command}* hola @${m.sender.split`@`[0]} a`, null, { mentions: [m.sender] })
  cm.key.fromMe = false 
